...
export default function AppointmentList() {
  ...
  const onCreated = (newAppointment) => {
    setShowModal(false);
    fetch();
  };
              ...
              {appointments.map((a) => (
                <tr key={a.id}>
                  ...
                  <td className="px-4 py-3 space-x-2">
                    {/* Doctor can approve/cancel assigned appointment */}
                    {user?.role === "DOCTOR" && a.doctor?.user?.id === user.userId && a.status === "PENDING" && (
                      <>
